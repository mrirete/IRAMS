...
| R-04 | Personal data reaching AI provider beyond expectation (free-text in prompts) | 2 | 2 | 4 | Server-side proxy only, per-workspace budget (0229), workspace AI-disable option, disclosure in privacy notice §4 | Monitor; `[DECIDE: verify Gemini tier terms]` |
| R-05 | Secret leakage via client bundle (VITE_ vars) or repo | 2 | 3 | **6** | `.env*` gitignored; service keys server-side only; `[BUILD: strip VITE_GEMINI_API_KEY fallback from prod builds — still present as dev fallback]`; CI secret scan `[BUILD: add gitleaks]` | Partially treated — two BUILDs open |
...
| R-08 | Account takeover of admin (no MFA yet) | 2 | 3 | **6** | `[BUILD: enable Supabase MFA/TOTP for SUPER_ADMIN/SYS_ADMIN roles — decided earlier, not yet built]`; strong-password policy; session revocation exists | **Open — top of treatment queue** |
| R-09 | Malicious/compromised dependency (supply chain) | 2 | 2 | 4 | Trivy daily; lockfiles; `[BUILD: npm audit gate + pin GH Actions by SHA]` | Partially treated |
| R-10 | Data loss (bad migration, deletion bug) | 2 | 3 | **6** | Transactional ledgered migrations (rollback on failure), Supabase PITR, `[DECIDE: annual restore test — schedule first one]` | Treated except restore-test evidence |
