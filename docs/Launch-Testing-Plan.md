...
| ~~Two migrations numbered `0234`~~ — 30 numbers were double-claimed, 0141 four times | 65 files across 30 numbers | R4 | ✅ fixed. Every migration now holds its own order slot (`0234_` then `0234a_`), the ledger moved with the renames, and the runner sorts on number **then suffix**. Verified: only the 3 groups with distinct `applied_at` changed position, and they moved to match what the ledger says actually ran |
