...
export function mapGoodsMovement(row: any): Mapped<GoodsMovementDoc> {
    ...
    return {
        doc: {
            ...
            // Business keys first, uuid only as a last resort — the receiver
            // can resolve WO-2026-4526; it can do nothing with 7f3a-…. The
            // assessment found this mapper shipping raw ids while
            // mapCostPosting shipped codes; 0277 put the keys on the view.
            storage_location_ref: opt(row?.location_code) || opt(row?.location_id),
            quantity: num(row?.quantity),
            unit: undefined,
            unit_cost: num(row?.cost_at_time),
            value: num(row?.total_value),
            work_order_ref: opt(row?.wo_number) || opt(row?.wo_id),
            purchase_order_ref: opt(row?.po_code) || opt(row?.po_id),
            cost_center_ref: opt(row?.cost_center_code) || opt(row?.cost_center_id),
            asset_ref: opt(row?.asset_tag) || opt(row?.asset_id),
...
export function mapPurchaseOrderLine(row: any): Mapped<PurchaseOrderLineDoc> {
    ...
    return {
        doc: {
            ...
            material_ref: opt(row?.material_number) || opt(row?.part_number),
            supplier_ref: opt(row?.supplier_code) || opt(row?.supplier_name) || opt(row?.supplier_id),
...
export function mapSupplierInvoice(row: any): Mapped<SupplierInvoiceDoc> {
    ...
    return {
        doc: {
            ...
            supplier_ref: opt(row?.vendor_code) || opt(row?.vendor_name) || opt(row?.vendor_id),
            purchase_order_ref: opt(row?.po_code) || opt(row?.po_id),
