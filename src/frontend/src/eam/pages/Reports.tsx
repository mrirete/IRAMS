  ...
  const renderOverview = () => (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 xs:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4">
        ...
        {/* Two DIFFERENT KPIs that used to share one name: PM share vs schedule
            adherence (PMs done by due date). The share is NOT SMRP 5.4.2
            Proactive Work (> 80%, which also counts PdM and corrective work
            found by PM/PdM) — that lives on Reliability Metrics. A plain PM
            count share maps to 5.1.4 PM Hours, whose 7th-edition target is
            15%, so the old 80% target was the wrong metric's target. */}
        <ReportKPICard title="PM Share" value={pmRatioCanonical.ratioPct ?? '—'} format={pmRatioCanonical.ratioPct == null ? undefined : 'percent'} subtitle="preventive ÷ all WOs · SMRP 5.1.4 hours target ≈15%; Proactive Work 5.4.2 is on Reliability Metrics" icon={<CheckCircle2 size={14} />} ragStatus="neutral" onClick={() => navigate('/reports/drilldown/pm-compliance')} clickLabel="View PM details" />
        ...
        <ReportKPICard title="MTTR" value={reliability.mttrHours ?? '—'} format={reliability.mttrHours == null ? undefined : 'hours'} subtitle={reliability.mttrHours == null ? 'no timed repairs' : `SMRP 3.5.2 · ${reliability.mttrBasis === 'repair' ? 'repair hours' : 'outage window as proxy'} · ${reliability.downtimeCoveragePct}% of failures timed`} icon={<Clock size={14} />} ragStatus={reliability.mttrHours == null ? 'neutral' : reliability.mttrHours <= 4 ? 'green' : reliability.mttrHours <= 8 ? 'amber' : 'red'} onClick={() => navigate('/reports/drilldown/mtbf-mttr')} clickLabel="View MTBF/MTTR analysis" />
        <ReportKPICard title="Availability (Ai)" value={reliability.availabilityPct ?? '—'} format={reliability.availabilityPct == null ? undefined : 'percent'} subtitle={reliability.availabilityPct == null ? 'needs failures + downtime' : 'inherent Ai = MTBF/(MTBF+MTTR) · SMRP G6.0'} target={95} targetLabel="Target: 95%" icon={<Gauge size={14} />} ragStatus={reliability.availabilityPct == null ? 'neutral' : reliability.availabilityPct >= 95 ? 'green' : reliability.availabilityPct >= 85 ? 'amber' : 'red'} onClick={() => navigate('/reports/drilldown/availability')} clickLabel="View Availability trend" />
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 xs:grid-cols-2 md:grid-cols-4 xl:grid-cols-7 gap-3 sm:gap-4">
          ...
          <ReportKPICard title="Avg MTTR" value={reliability.mttrHours ?? '—'} format={reliability.mttrHours == null ? undefined : 'hours'} subtitle={reliability.mttrHours == null ? 'no timed repairs in range' : `SMRP 3.5.2 · ${reliability.mttrBasis === 'repair' ? 'repair hours' : 'outage window as proxy'} · ${reliability.downtimeCoveragePct}% of failures timed`} icon={<Clock size={14} />} ragStatus={reliability.mttrHours == null ? 'neutral' : reliability.mttrHours <= 4 ? 'green' : reliability.mttrHours <= 8 ? 'amber' : 'red'} onClick={() => navigate('/reports/drilldown/mtbf-mttr')} clickLabel="View MTBF/MTTR analysis" />
          <ReportKPICard title="Availability (Ai)" value={reliability.availabilityPct ?? '—'} format={reliability.availabilityPct == null ? undefined : 'percent'} subtitle="inherent Ai = MTBF/(MTBF+MTTR) · SMRP G6.0" target={95} targetLabel="Target: 95%" icon={<Gauge size={14} />} ragStatus={reliability.availabilityPct == null ? 'neutral' : reliability.availabilityPct >= 95 ? 'green' : reliability.availabilityPct >= 85 ? 'amber' : 'red'} onClick={() => navigate('/reports/drilldown/availability')} clickLabel="View Availability trend" />
          {/* OEE needs production data. The fallback path invents performance
            (0.88) and quality (0.96); `isReal` existed but was never read, so
            a fabricated OEE rendered as fact. Now it says so. */}
        <ReportKPICard title="OEE" value={oeeData.isReal ? Number((oeeData.oee * 100).toFixed(1)) : '—'} format={oeeData.isReal ? 'percent' : undefined} subtitle={oeeData.isReal ? `from ${oeeData.assetCount} asset(s) logging production` : 'needs production data (rate & quality)'} target={85} targetLabel="Target: 85%" icon={<Activity size={14} />} ragStatus={!oeeData.isReal ? 'neutral' : oeeData.oee >= 0.85 ? 'green' : oeeData.oee >= 0.65 ? 'amber' : 'red'} onClick={() => navigate('/reports/drilldown/oee')} clickLabel="View OEE breakdown" />
