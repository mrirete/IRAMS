...
const QUICK_ACTIONS: Record<string, { label: string; icon: React.ReactNode; prompt: string }[]> = {
  ...
  // Analyze module (RCA / Defect Elimination / Pareto). Ported from the former
  // Analyze-only Specialist bubble so the one global panel keeps those skills.
  analyze: [
    { label: 'Recommend Tool', icon: <Sparkles size={12} />, prompt: 'Given the analysis context provided (active division, asset, and Pareto bad-actor summary), recommend the most appropriate analysis approach — 5-Why, Fishbone, Fault Tree, Logic Tree, FMEA, or Weibull. Justify the choice against the evidence available and state what evidence is still missing.' },
    { label: 'Generate Hypotheses', icon: <Search size={12} />, prompt: 'Propose candidate root-cause hypotheses for this failure. Enforce the Physical → Human → Latent causal ladder: the physical cause is never the root, so carry each hypothesis through to its systemic/organizational origin. Flag which hypotheses the current evidence supports and which are still speculative.' },
    { label: 'Corrective Actions', icon: <Wrench size={12} />, prompt: 'Based on the identified root causes, suggest corrective actions. For each, state the failure mode it eliminates, the expected reliability improvement, and how its effectiveness should be verified. Distinguish interim containment from permanent elimination.' },
    { label: 'Defect Pattern', icon: <BarChart3 size={12} />, prompt: 'Examine the failure history and Pareto data in context for repeat failure patterns. Identify chronic defects (recurring, low-cost-per-event but high cumulative cost) versus sporadic high-impact events, and recommend which to attack first on cost-of-unreliability grounds.' },
    { label: 'Draft DE Plan', icon: <Target size={12} />, prompt: 'Draft a defect elimination plan for the top bad actor in context: root-cause hypothesis, corrective actions, expected reliability improvement, estimated cost saving, and the verification method. Present it as a proposal for human review — do not assert it as decided.' },
  ],
  people: [
