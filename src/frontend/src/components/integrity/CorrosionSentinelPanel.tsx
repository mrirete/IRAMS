...
export const CorrosionSentinelPanel: React.FC<CorrosionSentinelPanelProps> = ({ assetTag }) => {
    ...
    return (
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-4 sm:px-5 py-3.5 border-b border-slate-100 bg-gradient-to-r from-primary-50 via-white to-white">
                <div className="flex items-center gap-2.5 min-w-0">
                    <span className="flex items-center justify-center w-8 h-8 rounded-lg bg-primary-100 text-primary-600 shrink-0"><Radar size={16} /></span>
                ...
                <div className="flex gap-2 sm:items-center shrink-0">
                    <input
                        ...
                        className="flex-1 sm:w-40 min-h-[40px] rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-primary-300 focus:border-primary-400"
                    />
                    <button
                        onClick={run}
                        disabled={loading}
