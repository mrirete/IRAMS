...
import { FlaskConical, Loader2, CheckCircle2, AlertTriangle, Radio } from 'lucide-react';
...
import {
    ...
    type PatternVerdict, type DecisionCheck, type ModeReconciliation, type CbmCoverageRow,
} from '../../lib/predict/rcmEvidence';
import type { RCMStudy, RCMFailureMode, RCMDecision } from '../../eam/services/RCMService';
...
export const RCMEvidencePanel: React.FC<Props> = ({ study, failureModes, decisions }) => {
    const [loading, setLoading] = useState(true);
    const [verdict, setVerdict] = useState<PatternVerdict | null>(null);
    const [recon, setRecon] = useState<ModeReconciliation | null>(null);
    const [coverage, setCoverage] = useState<CbmCoverageRow[]>([]);
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                if (!study.asset_id) { setVerdict(null); setRecon(null); setCoverage([]); return; }
                const [fit, defs, { data: wos }] = await Promise.all([
                ...
                // observed failure-mode counts from real WOs
                ...
                setRecon(reconcileModes(failureModes, counts));
                setCoverage(checkCbmCoverage(decisionList, defs || []));
            } finally {
                if (active) setLoading(false);
            }
        })();
        return () => { active = false; };
    }, [study.asset_id, failureModes, decisionList]);
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5">
                ...
                <div className="grid md:grid-cols-3 gap-3">
                    <div className="border border-red-200 bg-red-50/40 rounded-lg p-3">
                        <p className="text-[10px] font-bold text-red-600 uppercase tracking-wider mb-1.5">Observed, not analyzed ({recon?.observedNotAnalyzed.length ?? 0})</p>
                        {recon && recon.observedNotAnalyzed.length ? recon.observedNotAnalyzed.map(m => (
                            <p key={m.code} className="text-xs text-slate-700"><strong>{m.code}</strong> — {m.count}× on record · add to the study</p>
                    ...
                    <div className="border border-slate-200 bg-slate-50/60 rounded-lg p-3">
                        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Postulated, never observed ({recon?.postulatedNeverObserved.length ?? 0})</p>
                        {recon && recon.postulatedNeverObserved.length ? recon.postulatedNeverObserved.map(m => (
                    ...
                    <div className="border border-emerald-200 bg-emerald-50/40 rounded-lg p-3">
                        <p className="text-[10px] font-bold text-emerald-700 uppercase tracking-wider mb-1.5">Matched ({recon?.matched.length ?? 0})</p>
                        {recon && recon.matched.length ? recon.matched.map(m => (
