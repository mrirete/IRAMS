...
const sapEquipmentRegister: CmmsTemplate = {
    ...
    build: () => makeWorkbook(
        'Equipment List',
        ['Equipment', 'TechIdentNo.', 'Functional loc.', 'Superord.Equipment', 'Description of technical object', 'Object type',
        ...
        [
            ...
            ['10004530', 'PMP-101A-M', 'PLT1-CWS-PMP-101A', '10004521', 'Drive motor 45 kW - cooling water pump A', 'MOTOR', 'WEG', 'W22 225S/M', 'WEG-2019-31022', 'B'],
        ...
        [
            ...
            ['Functional loc.', 'The FL path (TPLNR) the equipment is installed at. Optional; kept as a reference property on the asset — this import does not build the location tree (see below).'],
            ['Superord.Equipment', 'The superordinate equipment (HEQUI) this item is installed in — a motor under its pump, a gearbox under its drive. Optional. When filled, the import sets the parent link, so equipment-to-subunit structure from SAP is kept. Either the superior EQUNR or its TechIdentNo. matches.'],
            [''],
...
const genericAssetRegister: CmmsTemplate = {
    ...
    build: () => makeWorkbook(
        'Asset Register',
        ['Asset Tag', 'Asset Name', 'Category', 'Parent Tag', 'Manufacturer', 'Model', 'Serial Number', 'Criticality'],
        [
            ['CWS-TRAIN-A', 'Cooling water system - train A', 'SYSTEM', '', '', '', '', 'A'],
            ['PMP-101A', 'Centrifugal pump - cooling water train A', 'PUMP', 'CWS-TRAIN-A', 'KSB', 'Etanorm 065-050', 'KSB-2019-04471', 'A'],
        ...
        [
            ...
            ['Parent Tag', 'Optional hierarchy: another row’s tag (or an asset already in IREAMS). Parents resolve AFTER all rows import, so order does not matter; a tag that matches nothing is reported and the asset imports without a parent. Deep multi-level trees with typed levels are still the Asset Register importer’s job — this column covers the common one-level "equipment under its system" case.'],
