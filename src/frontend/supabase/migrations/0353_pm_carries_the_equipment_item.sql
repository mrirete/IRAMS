-- 0353 — the PM and its work orders carry the equipment item the RCM decision is about.
--
-- A component pin on a failure mode reached the Specialist and the coverage
-- card, but not the work: the PM created from the decision named no subunit or
-- object part, so the work orders it generated could not be coded back to the
-- item and the living-study check could only match by failure-mode text.
--
--   * recurring_work.failure_mode_code / subunit_code / object_part — what the
--     PM prevents, on which item (ISO 14224 subunit + maintainable item /
--     object part). The client seed (generateWOFromPM) already read
--     failure_mode_code; the column never existed.
--   * pm_autogen_sweep copies all three into wo_failure_data on every generated
--     work order (was failure_mode_code + impacts only).
BEGIN;
...
COMMENT ON COLUMN public.recurring_work.failure_mode_code IS '0353: ISO 14224 failure mode this PM prevents — seeded onto every generated work order''s wo_failure_data.';
COMMENT ON COLUMN public.recurring_work.subunit_code IS '0353: ISO 14224 subunit (or the component tag) the PM addresses — seeded onto generated work orders.';
COMMENT ON COLUMN public.recurring_work.object_part IS '0353: the maintainable item / part the PM addresses — seeded onto generated work orders.';
