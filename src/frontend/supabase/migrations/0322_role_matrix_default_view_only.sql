-- ════════════════════════════════════════════════════════════════════════════
-- 0322 — Role matrix: the no-template default becomes view-only
--
-- BASE_PACKAGE_DEFAULTS is what a role code with no template resolves to —
-- 'USER' (which Grant System Access used to write), the legacy ELEC / R-ENG
-- codes, VENDOR, SUPPLIER, MANUFACTURER. It granted create + edit on assets,
-- requests, work orders, PMs and scheduling while calling itself fail-closed.
-- Now view-only, plus requests.create (anyone may raise a request).
--
-- ROLE_PERMISSION_TEMPLATES / BASE_PACKAGE_DEFAULTS changed in the same
-- commit; the seed below is REGENERATED, not hand-edited
-- (npm run gen:role-permissions). Re-seeds role_permissions wholesale — a new
-- migration because applied migrations are immutable (0253/0302/0312).
-- ════════════════════════════════════════════════════════════════════════════
BEGIN;
-- ── BEGIN GENERATED SEED (scripts/gen-role-permissions.mjs) ──
-- 779 permitted (role, module, action) triples. Do not hand-edit.
