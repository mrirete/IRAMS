-- ════════════════════════════════════════════════════════════════════════════
-- 0347 — Role matrix: permit to work follows the matrix
--
-- The PTW screen checked nothing: a technician could raise, approve (all four
-- roles), issue and activate their own permit (2026-09-08 assurance run).
-- The matrix gave every line role safety view-only, which was why nobody had
-- bothered to read it. Now: TECHNICIAN safety view+create (raise the request,
-- accept / return as performing authority); PLANNER safety view/create/edit
-- (prepare with the plan); SUPERVISOR safety view/create/edit/approve (area /
-- operations authority: approve, issue, suspend, close). 0348 enforces it on
-- ptw_permits / ptw_approvals.
--
