-- ════════════════════════════════════════════════════════════════════════════
-- 0323 — Work Management process gaps found by the role walk-through (2026-09-06)
--
-- Three layers were compared for every Work Management step — the role
-- templates, what each page's buttons check, and what RLS enforces:
--
--   • Requests could not pass Review for any line role: Authorize requires
--     requests.authorize and only SUPER/SYS_ADMIN held it. SUPERVISOR,
--     MANAGER and ASSET_MANAGER now authorise (template change, reseeded).
--   • A technician saw blank assignee / labour names on their own work
--     order: contacts SELECT enforces contacts.view and TECHNICIAN had
--     NO_ACCESS. Now VIEW_ONLY (template change, reseeded).
--   • "Complete & Raise Follow-Up" failed silently for a technician: the
--     follow-up corrective WO is a client insert, and rbac_insert_work_orders
--     needs workOrders.create, which TECHNICIAN does not hold. A follow-up is
--     a child of the order the technician is already executing — allow it on
--     workOrders.edit when parent_wo_id is set. PM-generated orders keep the
--     pm.view clause from 0248a.
--
-- ROLE_PERMISSION_TEMPLATES changed in the same commit; the seed below is
-- REGENERATED, not hand-edited (npm run gen:role-permissions).
-- ════════════════════════════════════════════════════════════════════════════
BEGIN;
-- ── BEGIN GENERATED SEED (scripts/gen-role-permissions.mjs) ──
-- 783 permitted (role, module, action) triples. Do not hand-edit.
...
CREATE POLICY rbac_insert_work_orders ON public.work_orders
    ...
    WITH CHECK (
        ...
        AND (
            ...
            OR (recurring_work_id IS NOT NULL AND (SELECT public.caller_can('pm', 'view')))
            OR (parent_wo_id IS NOT NULL AND (SELECT public.caller_can('workOrders', 'edit')))
        )
    );

COMMENT ON POLICY rbac_insert_work_orders ON public.work_orders IS
    'workOrders.create, or a PM-generated order (recurring_work_id) on pm.view, or a follow-up child (parent_wo_id) on workOrders.edit — the executing technician raises the corrective follow-up (0323).';

COMMIT;

-- VERIFY (after apply):
--   SELECT public.role_can('SUPERVISOR', '{}', 'requests', 'authorize');  -- true
--   SELECT public.role_can('TECHNICIAN', '{}', 'contacts', 'view');       -- true
--   SELECT public.role_can('TECHNICIAN', '{}', 'workOrders', 'create');   -- false (follow-up goes via parent_wo_id)
