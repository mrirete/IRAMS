-- ════════════════════════════════════════════════════════════════════════════
-- 0372 — Role matrix: FINANCE template
--
-- The full-loop assurance run (2026-09-19) had nobody to play the finance
-- role: no template existed, so the planner authorised, received and matched
-- the order they raised. FINANCE sees the modules the line roles see, holds
-- FinOps FULL minus delete, purchasing view + approve + authorize + viewCosts
-- (spending limit 50 000), viewCosts on inventory and work orders, and no
-- create/edit on work orders or scheduling. Pairs with the SoD refusal in
-- ers_authorize_purchase_order (0371): raiser ≠ authoriser.
--
-- Seed REGENERATED (npm run gen:role-permissions); applied migrations are
-- immutable, so this is a new wholesale re-seed.
-- ════════════════════════════════════════════════════════════════════════════
BEGIN;
-- ── BEGIN GENERATED SEED (scripts/gen-role-permissions.mjs) ──
-- 826 permitted (role, module, action) triples. Do not hand-edit.
...
-- Admin › People assigns roles from the CONTACT_TYPE dictionary (global rows,
-- company_id NULL); without a row the new template cannot be picked there.
INSERT INTO public.reference_codes (category, code, description, active, properties)
SELECT 'CONTACT_TYPE', 'FINANCE', 'Finance', true, '{"hourlyRate": 0}'::jsonb
 WHERE NOT EXISTS (SELECT 1 FROM public.reference_codes WHERE category = 'CONTACT_TYPE' AND code = 'FINANCE' AND company_id IS NULL);
COMMIT;
