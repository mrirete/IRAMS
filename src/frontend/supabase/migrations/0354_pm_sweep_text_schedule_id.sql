-- 0354 — the PM autopilot no longer dies on an RCM-generated schedule.
--
-- pm_autogen_sweep() returned schedule_id uuid and assigned recurring_work.id
-- to it. RCM-generated PMs have text ids ("RCM-<study>-<decision>"), so the
-- first due RCM PM raised 22P02 and aborted the WHOLE sweep — no calendar PM
-- in the tenant generated from the moment one RCM PM fell due (2026-09-08).
-- schedule_id is text now; nothing reads it as a uuid. The same-day package
-- absorption compared recurring_work.asset_id (text) with a uuid — cast added.
BEGIN;
