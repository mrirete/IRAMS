-- ════════════════════════════════════════════════════════════════════════════
-- 0302 — Role matrix, RF-01 repairs: RE sees money, supervisors see safety,
--         and the Asset Manager persona gets a home
--
-- Three findings from the role-fit register (RELANTERN-RF-01):
--   • RELIABILITY_ENG had viewCosts:false on every module — the persona the
--     reliability tier is sold to was blind to cost-ranked bad actors, DE
--     savings, and spares economics. Cost DISPLAY is now granted (dashboard,
--     assets, workOrders, pm, inventory, analytics); spending authority is
--     unchanged.
--   • SUPERVISOR safety was NO_ACCESS while technicians (who they review)
--     hold VIEW_ONLY — the same matrix-error class 0253 fixed for
--     TECHNICIAN. Supervisors now read the JSAs their crew completes.
--   • ASSET_MANAGER is a new template: MANAGER-grade cost/approval authority
--     plus the ISO 55000 stewardship surfaces (reliability & integrity
--     visibility with costs, audits approve, MoC approve/authorize, FinOps).
--
-- ROLE_PERMISSION_TEMPLATES (rolePermissions.ts) changed in the same commit;
-- the seed below is REGENERATED, not hand-edited (npm run gen:role-permissions).
-- It re-seeds role_permissions wholesale — a new migration because applied
-- migrations are immutable (0253 precedent; the runner's checksum guard).
-- ════════════════════════════════════════════════════════════════════════════

BEGIN;

-- ── BEGIN GENERATED SEED (scripts/gen-role-permissions.mjs) ──
-- 760 permitted (role, module, action) triples. Do not hand-edit.
...
-- VERIFY (after apply):
--   SELECT count(*) FROM role_permissions WHERE role = 'ASSET_MANAGER';           -- > 0
--   SELECT 1 FROM role_permissions WHERE role='RELIABILITY_ENG' AND module='analytics' AND action='viewCosts';
--   SELECT 1 FROM role_permissions WHERE role='SUPERVISOR' AND module='safety' AND action='view';
