-- ════════════════════════════════════════════════════════════════════════════
-- 0312 — Role matrix: STOREKEEPER
--
-- Launch review (RELANTERN-LR-01): two seeded notification rules addressed a
-- "Storekeeper" while no such role template existed, so anyone given that
-- role fell back to the fail-closed defaults — view-only inventory, unable to
-- receive goods or adjust stock. The template now exists in rolePermissions.ts
-- (inventory full, purchasing basic, work orders view, Specialist view per the
-- product ruling that every role can open it).
--
-- ROLE_PERMISSION_TEMPLATES changed in the same commit; the seed below is
-- REGENERATED, not hand-edited (npm run gen:role-permissions). It re-seeds
-- role_permissions wholesale — a new migration because applied migrations are
-- immutable (0253 / 0302 precedent; the runner's checksum guard).
-- ════════════════════════════════════════════════════════════════════════════
BEGIN;
-- ── BEGIN GENERATED SEED (scripts/gen-role-permissions.mjs) ──
-- 790 permitted (role, module, action) triples. Do not hand-edit.
